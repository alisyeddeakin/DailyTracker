    ...
    protected String saveData() {
        ...
        try {
            ...
            for (int i = 0; i < data.getXValCount(); i++) {
                row = new Row();
                String rowValue = String.format(Locale.US, "%.3f,%.3f,%.3f,%.3f%n", i * samplePeriod,
                        xSpinDataSet.getEntryForXIndex(i).getVal(),
                        ySpinDataSet.getEntryForXIndex(i).getVal(),
                        zSpinDataSet.getEntryForXIndex(i).getVal());
                fos.write(rowValue.getBytes());

                row.setTime(String.valueOf( (i*samplePeriod) ));
                row.setX_axis(String.valueOf(xSpinDataSet.getEntryForXIndex(i).getVal()));
                row.setY_axis(String.valueOf(ySpinDataSet.getEntryForXIndex(i).getVal()));
                row.setZ_axis(String.valueOf(zSpinDataSet.getEntryForXIndex(i).getVal()));

                rows.add(row);

            }
            Utils.dataUploadable = rows;
            fos.close();
            return filename;
        } catch (Exception e) {
            e.printStackTrace();
